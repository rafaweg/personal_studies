...
SELECT
  ...
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 30 THEN vlWeightGramas
                ELSE 0 END), 0) AS vlAvgWeight1M,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 90 THEN vlWeightGramas
                ELSE 0 END), 0) AS vlAvgWeight3M,
  avg(nrNameLength) AS vlAvgNameLength,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 30 THEN nrNameLength
                ELSE 0 END), 0) AS vlAvgNameLengtht1M,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 90 THEN nrNameLength
                ELSE 0 END), 0) AS vlAvgNameLength3M,
  avg(nrPhotos) AS vlAvgPhotos,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 30 THEN nrPhotos
                ELSE 0 END), 0) AS vlAvgPhotos1M,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 90 THEN nrPhotos
                ELSE 0 END), 0) AS vlAvgPhotos3M,
  avg(vlLengthCm * vlHeightCm * vlWidthCm) AS vlAvgProductVolume,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 30 THEN (vlLengthCm * vlHeightCm * vlWidthCm)
                ELSE 0 END), 0) AS vlAvgProductVolume1M,
  COALESCE(avg(CASE
                WHEN datediff('2018-01-01',dtPurchase) < 90 THEN (vlLengthCm * vlHeightCm * vlWidthCm)
                ELSE 0 END), 0) AS vlAvgProductVolume3M,
  count(DISTINCT descCategoryName) AS qtCategoryType,
  count(DISTINCT CASE
                  WHEN datediff('2018-01-01',dtPurchase) < 30 THEN descCategoryName END) AS qtCategoryType1M,
  count(DISTINCT CASE
                  WHEN datediff('2018-01-01',dtPurchase) < 90 THEN descCategoryName END) AS qtCategoryType3M,
  count(DISTINCT idProduct) AS qtProducts,
  count(DISTINCT CASE
                  WHEN datediff('2018-01-01',dtPurchase) < 30 THEN idProduct END) AS qtProducts1M,
  count(DISTINCT CASE
                  WHEN datediff('2018-01-01',dtPurchase) < 90 THEN idProduct END) AS qtProducts3M
FROM
  full_table
GROUP BY
  idSeller
